...
class locators_schedule:
    ...
    LOCATOR_WRITE_TO_DOCTOR = (By.XPATH, '//tbody/tr[4]//a[contains(text(), "Записать")]')
...
class schedule(BasePage):
    def patient_schedule(self):
        try:
            ...
            self.find_element_pb() # прогрессбар
            self.find_element(locators_schedule.LOCATOR_CHOOSE_DOCTOR).click() # выбор врача
            self.find_element_pb() # прогрессбар
            ...
            search_string_4.send_keys(prm.patient) # вввод карты
