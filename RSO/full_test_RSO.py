import testit
from RSO.page_object.login_page import login
from RSO.page_object.doctors_diary_page import doctors_diary
from RSO.page_object.schedule_page import schedule
from RSO.page_object.hospitalization_page import hospitalization
from RSO.page_object.search_patient_page import search_patient
# import time


@testit.step('Модуль: Авторизация', 'Проверка авторизации на продуктивном стенде')
def test_RSO_login(browser_RSO):
    start_page = login(browser_RSO)
    start_page.auth()


@testit.step('Модуль: Дневник врача', 'Запись пациента, оказание услуги и ее отмена')
def test_RSO_doctors_diary(browser_RSO):
    # test_RSO_login(browser_RSO)  # тест авторизации
    doctors_diary_test = doctors_diary(browser_RSO)
    doctors_diary_test.diary()


@testit.step('Модуль: Расписание', 'Проверка записи пациента к врачу')
def test_RSO_schedule(browser_RSO):
    # test_RSO_login(browser_RSO)  # тест авторизации
    patient_schedule_test = schedule(browser_RSO)
    patient_schedule_test.patient_schedule()


@testit.step('Модуль: Госпитализация', 'Госпитализация пользователя')
def test_RSO_hospitalization(browser_RSO):
    # test_RSO_login(browser_RSO)  # тест авторизации
    patient_hospitalization_test = hospitalization(browser_RSO)
    patient_hospitalization_test.register_patient()


@testit.step('Модуль: Поиск пациента', 'Создание и удаление тестового пациента')
def test_RSO_search_patient(browser_RSO):
    # test_RSO_login(browser_RSO)  # тест авторизации
    search_patient_test = search_patient(browser_RSO)
    search_patient_test.create_patient()

# def test_RSO_full(self, browser_RSO):
#     start = time.time()  # начало отсчета
#     start_page = login(browser_RSO)  # тест модуля авторизации
#     start_page.auth()
#     start_doctors_diary = time.time()
#     doctors_diary_test = doctors_diary(browser_RSO)  # тест модуля "Дневник врача"
#     doctors_diary_test.diary()
...
#     patient_schedule_test = schedule(browser_RSO)  # тест модуля "Расписание"
#     patient_schedule_test.patient_schedule()
...
#     patient_hospitalization_test = hospitalization(browser_RSO)  # тест модуля "Госпитализация"
#     patient_hospitalization_test.register_patient()
...
#     search_patient_test = search_patient(browser_RSO)  # тест модуля "Поиск пациента"
#     search_patient_test.create_patient()
